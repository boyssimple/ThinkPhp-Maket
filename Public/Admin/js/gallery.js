$(function(){
    $('#datatable1').DataTable({
        ...
        serverSide: true,
        ajax: ENV+"/Gallery/loadList",
        aoColumns : [//服务器返回的数据处理 此时返回的是 {}
            { "mData": "id","sWidth": 80},
                                        { "mData": "url","sWidth": 200},                    { "mData": "remark","sWidth": 200},
            { "mData": function(obj){
...
function edit(id){
    window.location.href = ENV + '/Gallery/add?id='+id;
}

function del(id){
    if (window.confirm("确定删除？")){
        $.post(ENV + '/Gallery/delete',{id:id},function(result){
