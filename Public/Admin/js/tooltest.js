$(function(){
    $('#datatable1').DataTable({
        ...
        serverSide: true,
        ajax: ENV+"/Tooltest/loadList",
        aoColumns : [//服务器返回的数据处理 此时返回的是 {}
            { "mData": "id","sWidth": 80},
                                        { "mData": "name","sWidth": 200},                    { "mData": "remark","sWidth": 200},
            { "mData": function(obj){
...
function edit(id){
    window.location.href = ENV + '/Tooltest/add?id='+id;
}

function del(id){
    if (window.confirm("确定删除？")){
        $.post(ENV + '/Tooltest/delete',{id:id},function(result){
