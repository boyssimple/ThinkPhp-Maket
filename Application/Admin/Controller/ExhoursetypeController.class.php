...
class ExhoursetypeController extends CommonController {
    ...
    public function save(){
        if (IS_GET){
            ...
            $data = I('get.');
            $model = D("Exhoursetype");
            $id = $data['id'];
            if (empty($id)){
                $model->data($data)->add();
                $this->success('新增成功!', __APP__.'/Exhoursetype/add');
            }else{
                $model->data($data)->save();
                $this->success('修改成功!', __APP__.'/Exhoursetype/index');
            }
        }else{
            $this->error('参数传递错误!', __APP__.'/Exhoursetype/add');
        }
    }
